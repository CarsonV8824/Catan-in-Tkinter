...
class Tabs:
    ...
    def dice_tab(
        ...
        self.dice_button = ttk.Button(
            frame,
            text="Roll Dice",
            command=lambda: game_loop.game_turn(
                self.dice_button,
                player_turn_info,
                players=players,
                first_dice_label=first_dice,
                second_dice_label=second_dice,
                total_of_dice_label=total_of_dice,
        ...
        player_turn_info = ttk.Label(
            frame, text=f"{players[game_loop.player_index].name}'s Turn"
        )
    ...
    def player_stats_tab(self, notebook: ttk.Notebook, players: list):
        ...
        scrollbar = ttk.Scrollbar(
            player_stats_tab, orient="vertical", command=canvas.yview
        )
        scrollable_frame = ttk.Frame(canvas)
        ...
        for player in players:
            ...
            self.player_stat_vars[player.name] = {
                "color": color_var,
    ...
    def update_player_stats(
        self, players: list[Player], game_loop: GameLoop, game_struct: GameStruct
    ):
        for player in players:
            player: Player = player
        ...
        self.player_ports = game_struct.get_all_available_ports_from_player(
            players[game_loop.player_index].name
        )
    ...
    def trade_tab(
        ...
        first_trade_player = ttk.Combobox(
            player_player_frame,
        ...
        second_trade_player = ttk.Combobox(
            player_player_frame,
            values=[player.name for player in players],
            state="readonly",
        )
        ...
        def on_select_first(event):
            ...
            self.first_trade_player_text = ttk.Label(
                player_player_frame, text=f"{self.first_trade_player}'s Resources:"
            )
            self.first_trade_player_text.pack(padx=10, pady=10)
            self.first_player_items = ttk.Treeview(
                player_player_frame, selectmode="extended", show="tree"
            )
            ...
            try:
                ...
                pass
            for resource, amount in next(
                player for player in players if player.name == self.first_trade_player
            ).resources.items():
        ...
        def on_select_second(event):
            ...
            try:
                ...
                pass
            self.second_trade_player_text = ttk.Label(
                player_player_frame, text=f"{self.second_trade_player}'s Resources:"
            )
            self.second_trade_player_text.pack(padx=10, pady=10)
            self.second_player_items = ttk.Treeview(
                player_player_frame, selectmode="extended", show="tree"
            )
            ...
            try:
                ...
                pass
            for resource, amount in next(
                player for player in players if player.name == self.second_trade_player
            ).resources.items():
                for _ in range(amount):
                    if resource != "victory_points" and resource != "knight_cards":
                        try:
            ...
            self.second_player_items.config(
                height=(
                    len(self.second_player_items.get_children())
                    if len(self.second_player_items.get_children()) > 0
                    else 1
                )
            )
        ...
        def get_trade_selections():
            ...
            # Update heights
            self.first_player_items.config(
                height=len(self.first_player_items.get_children()) or 1
            )
            self.second_player_items.config(
        ...
        trade_in_label = ttk.Label(
            trade_in_frame, text="Trade in 4 of the same resource for 1 of your choice"
        )
        trade_in_label.pack(pady=5)

        trade_in_button = ttk.Button(
            trade_in_frame,
            text="Trade In Resources",
            command=lambda: self.trade_in_resources(players, game_loop, game_struct),
        )
        ...
        self.select_ports = ttk.Combobox(
            ports_frame, values=self.player_ports, state="readonly"
        )
        self.select_ports.pack(pady=5)

        self.trade_in_port_btn = ttk.Button(
            ports_frame,
            command=lambda: confirm_trade(
                players[game_loop.player_index],
                game_loop=game_loop,
        ...
        def trade_in(
            player: Player,
            resorce_box: ttk.Combobox,
            type_of_port: str,
            resorce_to_trade_in_for_box: ttk.Combobox,
        ):
        ...
        def confirm_trade(player: Player, game_loop: GameLoop, game_struct: GameStruct):
            ...
            match not_last_port_type:
                ...
                case "PortNorm":
                    ttk.Label(
                        port_toplevel, text="Trade 3 of the same card for any card!"
                    ).pack()
                ...
                case "PortGreen":
                    ttk.Label(
                        port_toplevel, text="Trade 2 Green cards for any card!"
                    ).pack()

                    combo = ttk.Combobox(
                        port_toplevel, values=["green"], state="readonly"
                    )
                    combo.pack()
                ...
                case "PortYellow":
                    ttk.Label(
                        port_toplevel, text="Trade 2 Yellow cards for any card!"
                    ).pack()

                    combo = ttk.Combobox(
                        port_toplevel, values=["yellow"], state="readonly"
                    )
                    combo.pack()
                ...
                case "PortBrown":
                    ttk.Label(
                        port_toplevel, text="Trade 2 Brown cards for any card!"
                    ).pack()

                    combo = ttk.Combobox(
                        port_toplevel, values=["brown"], state="readonly"
                    )
                    combo.pack()
                ...
                case "PortLime":
                    ttk.Label(
                        port_toplevel, text="Trade 2 Lime cards for any card!"
                    ).pack()

                    combo = ttk.Combobox(
                        port_toplevel, values=["lime"], state="readonly"
                    )
                    combo.pack()
    ...
    def trade_in_resources(
        ...
        trade_in_text = ttk.Label(
            self.trade_in_resources_top, text=f"Select 4 resources to trade in"
        )
        trade_in_text.pack(pady=10)

        trade_in_text.config(
            text=f"Select 4 resources to trade in {players[game_loop.player_index].name}"
        )

        choose_texture_box_from_you = ttk.Label(
            self.trade_in_resources_top, text="Choose resource to give:"
        )
        choose_texture_box_from_you.pack(pady=10)

        choose_resource_box_from_you = ttk.Combobox(
            self.trade_in_resources_top,
            values=["lime", "green", "brown", "yellow", "gray"],
            state="readonly",
        )
        choose_resource_box_from_you.pack(pady=10)

        choose_texture_box_from_bank = ttk.Label(
            self.trade_in_resources_top, text="Choose resource to receive:"
        )
        choose_texture_box_from_bank.pack(pady=10)

        choose_resource_box_from_bank = ttk.Combobox(
            self.trade_in_resources_top,
            values=["lime", "green", "brown", "yellow", "gray"],
            state="readonly",
        )
        ...
        def confirm_trade_in():
            ...
            if (
                players[game_loop.player_index].get_resource_count(chosen_resource_you)
                < 4
            ):
                trade_in_text.config(
                    text=f"Not enough {chosen_resource_you} to trade in."
                )
                return
        ...
        confirm_trade_in_button = ttk.Button(
            self.trade_in_resources_top,
            text="Confirm Trade In",
            command=confirm_trade_in,
        )
    ...
    def buy_tab(
        ...
        draw_card_button = ttk.Button(
            frame,
            text="Draw Card",
            command=lambda: self.choose_development_card(
                players,
                game_loop.player_index,
                lambda players, button: game_loop.place_robber(players, button),
                lambda players, button: game_loop.place_two_roads(players, button),
                game_loop=game_loop,
                game_struct=game_struct,
            ),
        )
    ...
    def choose_development_card(
        ...
        if (
            players[current_player_index].resources["lime"] < 1
            and players[current_player_index].resources["gray"] < 1
            and players[current_player_index].resources["yellow"] < 1
        ):
            self.card_result_label.config(
                text="Not enough resources to buy a development card."
            )
            return
        ...
        match chosen_card:
            ...
            case "Year of Plenty":
                ...
                ttk.Label(
                    year_of_plenty_top, text="You may choose 2 resources of your choice"
                ).pack(pady=10)
            ...
            case "Monopoly":
                ...
                ttk.Label(monolopy_top, text="Choose a resource to monopolize:").pack(
                    pady=10
                )
                    ...
                    for i, player in enumerate(players):
                        if i != current_player_index:
                            ...
                            if amount > 0:
                                ...
                                players[current_player_index].add_resource(
                                    resource, amount
                                )
    ...
    def additional_info_tab(self, notebook: ttk.Notebook):
        ...
        scrollbar_y = ttk.Scrollbar(
            past_games_tab, orient="vertical", command=canvas.yview
        )
        scrollbar_x = ttk.Scrollbar(
            past_games_tab, orient="horizontal", command=canvas.xview
        )
        scrollable_frame = ttk.Frame(canvas)
